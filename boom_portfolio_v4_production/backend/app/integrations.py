...
# Google is used only for private Drive access by the photography CMS.
...
def oauth_state(admin_email: str):
    s = get_settings()
    return URLSafeTimedSerializer(s.oauth_state_secret).dumps({"email": admin_email})


def decode_state(state: str):
    try:
        return URLSafeTimedSerializer(get_settings().oauth_state_secret).loads(state, max_age=900)
...
def save_google_token(admin_email: str, token: dict):
    ...
    if existing.data and not token.get("refresh_token"):
        try:
            ...
            if old.get("scope") and not token.get("scope"):
    ...
    (
        ...
        .upsert(
            ...
            on_conflict="provider,owner_email",
        )
        .execute()
    )
...
def load_google_token(admin_email: str):
    ...
    if not result.data:
        return None
    try:
        return json.loads(_fernet().decrypt(result.data[0]["encrypted_token"].encode()))
    except Exception:
        return None
...
async def google_access_token(admin_email: str):
    ...
    saved = float(token.get("saved_at") or 0)
    ...
    if response.is_error:
        raise HTTPException(401, "Google connection expired; reconnect Google")
    new = response.json()
    new["refresh_token"] = refresh
    if token.get("scope") and not new.get("scope"):
